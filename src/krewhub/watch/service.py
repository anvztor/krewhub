...
class WatchService:
    ...
    async def notify_ephemeral(
        ...
        the UI wants live but that must not bloat the durable replay buffer
        (R1, alert#34: heartbeats were 84% of watch_log). On reconnect the
        client re-derives current state from the API, so a dropped ephemeral
        event is harmless. The event carries the current max durable seq so
        it never advances or regresses a subscriber's replay cursor.
        """
