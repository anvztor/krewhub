...
class TestReconcileSpawning:
    ...
    async def test_completed_bundle_is_not_rerun(self):
        """Regression (alert#34): a bundle whose graph ran to a terminal
        outcome must NOT be returned by list_runnable again, or the
        level-triggered runner re-executes it every reconcile cycle
        forever — emitting an unbounded stream of identical milestone
        events (1M+ rows / 5GB observed in prod). Bundle stays OPEN
        (step d.1) but is excluded from the runnable set via its
        terminal marker.
        """
        bundle_id, task_ids = await _seed_runnable_bundle()
        db = await get_db()
        ...
        try:
            ...
            assert bundle.status == BundleStatus.OPEN
            # But the runner must have stamped a terminal marker so the
            # next reconcile cycle skips it.
